		...
		<main className='w-full sm:w-115 flex flex-col gap-10 items-center'>
			...
			<section className='w-full px-6 py-5 flex flex-col gap-6 items-center justify-center bg-slate-800 rounded-2xl slate-shadow'>
				...
				<div className='flex flex-row items-center p-2 rounded-base bg-slate-900 w-full'>
					...
					<button
						className={`w-full py-3 flex items-center justify-center rounded-base cursor-pointer ${mark === 'O' ? 'bg-slate-300' : 'bg-transparent hover:bg-slate-850'}`}
						onClick={() => setMark('O')}>
			...
			<section className='flex flex-col gap-5 w-full'>
				<button
					className='text-preset-4 sm:text-preset-3 text-slate-900 rounded-2xl p-4 bg-amber-400 hover:bg-amber-300 cursor-pointer amber-shadow uppercase'
					onClick={() => startGameWithCPU(mark)}>
					New Game (vs CPU)
				</button>
				<button
					className='text-preset-4 sm:text-preset-3 text-slate-900 rounded-2xl p-4 bg-teal-400 hover:bg-teal-300 cursor-pointer teal-shadow uppercase'
